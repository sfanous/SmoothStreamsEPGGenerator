...
class Rovi(object):
    ...
    def _generate_xmltv_file(cls, country, postal_code):
        ...
        completed_process = subprocess.run(
            [
                ...
                '-r',
                '{0}:{1}'.format(
                    Configuration.get_configuration_parameter('ROVI_API_KEY'),
    ...
    def _request_template_json(cls):
        url = '{0}'.format(
            DEFAULT_ROVI_TEMPLATE_URL.format(
                Configuration.get_configuration_parameter('ROVI_API_KEY')
            )
        )
        ...
        response = Utility.make_http_request(
            session.get, url, headers=session.headers, auth=auth
        )
