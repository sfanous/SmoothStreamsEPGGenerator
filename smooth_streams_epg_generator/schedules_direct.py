...
class SchedulesDirect(object):
    ...
    def _do_generate_xmltv_file(cls, country):
        ...
        for xmltv_file_name in os.listdir(DEFAULT_MC2XML_OUTPUT_DIRECTORY_PATH):
            if xmltv_file_name == '{0}.xml'.format(country):
                xmltv_file_attributes = os.stat(
                    os.path.join(DEFAULT_MC2XML_OUTPUT_DIRECTORY_PATH, xmltv_file_name)
                )
                xmltv_file_modification_date_time_in_local = datetime.fromtimestamp(
                    xmltv_file_attributes.st_mtime
                ).astimezone(tzlocal.get_localzone())

                return (
                    current_date_time_in_local
                    > xmltv_file_modification_date_time_in_local + timedelta(hours=18)
                )
    ...
    def _generate_xmltv_file(cls, country, postal_code, lineup_name):
        ...
        completed_process = subprocess.run(
            [
                '{0}'.format(
                    os.path.join(
                        DEFAULT_MC2XML_DIRECTORY_PATH,
                        DEFAULT_MC2XML_EXECUTABLE_MAP[platform.system()],
                    )
                ),
                '-F',
                '-u',
                ...
                '-24',
                '-c',
                '{0}'.format(country[0:2]),
                '-g',
                '{0}'.format(postal_code),
                '-J',
                '{0}:{1}'.format(
                ...
                '-D',
                '{0}'.format(
                    os.path.join(DEFAULT_MC2XML_DIRECTORY_PATH, country, 'mc2xml.dat')
                ),
                '-C',
                '{0}'.format(
                    os.path.join(DEFAULT_MC2XML_DIRECTORY_PATH, country, 'mc2xml.chl')
                ),
                '-o',
                '{0}'.format(
                    os.path.join(
                        DEFAULT_MC2XML_OUTPUT_DIRECTORY_PATH, '{0}.xml'.format(country)
                    )
                ),
            ],
        ...
        if completed_process.returncode:
            error = (
                'Failure while executing mc2xml\n'
                'Executed command => %s\n'
                'Standard output  => %s\n'
                'Standard error   => %s',
    ...
    def _populate_lineups(cls):
        logger.debug(
            'Obtaining SchedulesDirect token\n'
            'URL      => %s\n'
            'username => %s\n'
            'password => %s',
        ...
        if response.status_code == requests.codes.OK:
            if token_response['code'] == 0:
                ...
                logger.debug(
                    'Listing lineups currently added to the account\n'
                    'URL   => %s\n'
                    'token => %s',
    ...
    def generate_xmltv_files(cls):
        ...
        schedules_direct_listings = Configuration.get_configuration_parameter(
